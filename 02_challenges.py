...
def test_raise_to_power():
    assert raise_to_power(10, 3) == 1000, \
        format_err_msg(1000, raise_to_power(10, 3))
    assert raise_to_power(25, 2) == 625, \
        format_err_msg(625, raise_to_power(25, 2))
    assert raise_to_power(10, 0) == 1, \
        format_err_msg(1, raise_to_power(10, 0))
...
def test_is_multiple_of_6():
    assert is_multiple_of_6(6) is True, \
        format_err_msg(True, is_multiple_of_6(6))
    assert is_multiple_of_6(10) is False, \
        format_err_msg(False, is_multiple_of_6(10))
    assert is_multiple_of_6(15) is False, \
        format_err_msg(False, is_multiple_of_6(15))
    assert is_multiple_of_6(36) is True, \
        format_err_msg(True, is_multiple_of_6(36))
    assert is_multiple_of_6(60) is True, \
        format_err_msg(True, is_multiple_of_6(60))
    assert is_multiple_of_6(61) is False, \
        format_err_msg(False, is_multiple_of_6(61))
...
def test_capitalise_first_letter():
    assert capitalise_first_letter("bang") == "Bang", \
        format_err_msg("Bang", capitalise_first_letter("bang"))
    assert capitalise_first_letter("apple") == "Apple", \
        format_err_msg("Apple", capitalise_first_letter("apple"))
    assert capitalise_first_letter("coding") == "Coding", \
        format_err_msg("Coding", capitalise_first_letter("coding"))
...
def is_in_the_20th_century(year):
    return (year - 1) // 100 == 19 


@run_test
def test_is_in_the_20th_century():
    assert is_in_the_20th_century(1962) is True, \
        format_err_msg(True, is_in_the_20th_century(1962))
    assert is_in_the_20th_century(1901) is True, \
        format_err_msg(True, is_in_the_20th_century(1901))
    assert is_in_the_20th_century(1900) is False, \
        format_err_msg(False, is_in_the_20th_century(1900))
    assert is_in_the_20th_century(1913) is True, \
        format_err_msg(True, is_in_the_20th_century(1913))
    assert is_in_the_20th_century(1876) is False, \
        format_err_msg(False, is_in_the_20th_century(1876))
    assert is_in_the_20th_century(2001) is False, \
        format_err_msg(False, is_in_the_20th_century(2001))
    assert is_in_the_20th_century(2000) is True, \
        format_err_msg(True, is_in_the_20th_century(2000))
...
def get_char_code(c):
    return f"The ASCII code for {c} is {ord(c)}"


@run_test
def test_get_char_code():
    assert get_char_code("A") == "The ASCII code for A is 65", \
        format_err_msg("The ASCII code for A is 65", get_char_code("A"))
    assert get_char_code("b") == "The ASCII code for b is 98", \
        format_err_msg("The ASCII code for b is 98", get_char_code("b"))
    assert get_char_code("z") == "The ASCII code for z is 122", \
        format_err_msg("The ASCII code for z is 122", get_char_code("z"))
    assert get_char_code("k") == "The ASCII code for k is 107", \
        format_err_msg("The ASCII code for k is 107", get_char_code("k"))
    assert get_char_code("!") == "The ASCII code for ! is 33", \
        format_err_msg("The ASCII code for ! is 33", get_char_code("!"))
    assert get_char_code("M") == "The ASCII code for M is 77", \
        format_err_msg("The ASCII code for M is 77", get_char_code("M"))
...
def test_create_list():
    assert create_list(3, "!") == ["!", "!", "!"], \
        format_err_msg(["!", "!", "!"], create_list(3, "!"))
    assert create_list(5, "a") == ["a", "a", "a", "a", "a"], \
        format_err_msg(["a", "a", "a", "a", "a"], create_list(5, "a"))
...
def check_battery_level(level):
    if level <= 5:
        return f"Warning - battery level low: {level}%, please charge your device"
    elif level <= 99:
...
def test_check_battery_level():
    assert check_battery_level(100) == "Fully charged :)", \
        format_err_msg("Fully charged :)", check_battery_level(100))

    assert check_battery_level(99) == "Battery level: 99%", \
        format_err_msg("Battery level: 99%", check_battery_level(99))
    assert check_battery_level(80) == "Battery level: 80%", \
        format_err_msg("Battery level: 80%", check_battery_level(80))
    assert check_battery_level(30) == "Battery level: 30%", \
        format_err_msg("Battery level: 30%", check_battery_level(30))
    assert check_battery_level(10) == "Battery level: 10%", \
        format_err_msg("Battery level: 10%", check_battery_level(10))
    assert check_battery_level(6) == "Battery level: 6%", \
        format_err_msg("Battery level: 6%", check_battery_level(6))
...
def collect_strings(lst):
    return [element for element in lst if type(element) == str]
